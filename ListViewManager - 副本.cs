...
namespace OSPE
{
    ...
    public partial class MainForm
    {
        ...
        public static string UnicodeToGbk(string unicodeStr)
        {
            ...
            byte[] unicodeBytes = Encoding.Unicode.GetBytes(unicodeStr);
            byte[] gbkBytes = Encoding.Convert(Encoding.Unicode, GbkEncoding, unicodeBytes);
        ...
        public void AddPacket(Packet packet)
        {
            ...
            //byte[] utf8Bytes = Encoding.Unicode.GetBytes(data);
            //byte[] gbkBytes = new byte[bufSize];
            //for (int i = 0; i < bufSize; i++)
            //{
            //    gbkBytes[i] = utf8Bytes[i * 2];
            //}
            //string str4 = Encoding.GetEncoding("GBK").GetString(gbkBytes);
            // 获取并截断数据
            var rawData = Settings.ViewAsHex ? packet.GetBufferAsHex() : packet.GetBufferAsText();
            var truncatedData = rawData.Length > ColDataTextLength? rawData.Substring(0, ColDataTextLength): rawData;

            // 转换为GBK编码
            //byte[] unicodeBytes = Encoding.Unicode.GetBytes(truncatedData);
            //int targetLength = truncatedData.Length;
            //byte[] gbkBytes = new byte[targetLength];
            ...
            //string result = Encoding.GetEncoding("GBK").GetString(gbkBytes, 0, copyLength);

            string result = ConvertUnicodeToGbk(truncatedData);
